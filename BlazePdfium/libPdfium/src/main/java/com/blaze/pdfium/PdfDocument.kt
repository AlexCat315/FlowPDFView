...
/**
 * PDF 文档类，支持线程安全的操作
 * 所有文档操作都受内部互斥锁保护以确保原子状态更改
 *
 * @property pageCache 已加载页面的引用缓存，包含访问计数
 * @property textPageCache 已加载文本页面的引用缓存，包含访问计数
 * @property nativePtr PDF 文档的原生指针（JNI 引用）
 * @property fileDescriptor PDF 内容的文件描述符
 */
class PdfDocument : Closeable {
    ...
    fun openTextPages(start: Int, end: Int): List<PdfTextPage> {
        ...
        val textPages: List<PdfTextPage>
        synchronized(lock = PdfiumCore.lock) {
            textPages = (start..end).map { index: Int ->
                val pagePtr: Long = pageCache[index]?.pagePtr ?: 0L
                val textPagePtr: Long = nativeLoadTextPage(docPtr = nativePtr, pagePtr = pagePtr)
                textPageCache[index] = PageCount(pagePtr = textPagePtr, count = 1)
                PdfTextPage(
                    doc = this@PdfDocument,
                    pageIndex = index,
                    textPagePtr = textPagePtr,
