...
static void destroyLibraryIfNeed() {
    ...
    // Note: Never actually destroy the library to avoid V8 crash from destroy+init sequence.
    // The library stays resident in memory for the lifetime of the process.
    // This is acceptable since PDF rendering is typically a short-lived operation.
}
...
JNI_FUNC(jlong, PdfiumCore, nativeOpenDocument)(JNI_ARGS, jint fd, jstring password) {
    ...
    if (!document) {
        ...
        if (errorNum == FPDF_ERR_PASSWORD) {
            jniThrowException(env, "com/blaze/pdfium/PdfPasswordException",
...
JNI_PdfDocument(jboolean, PdfiumCore, nativeSaveAsCopy)(JNI_ARGS, jlong docPtr, jobject callback,
                                                        jint flags) {
    jclass callbackClass = env->FindClass("com/blaze/pdfium/PdfWriteCallback");
...
JNI_FUNC(jobject, PdfiumCore, nativeGetPageSizeByIndex)(JNI_ARGS, jlong docPtr, jint pageIndex, jint dpi) {
    ...
    jclass clazz = env->FindClass("com/blaze/pdfium/util/Size");
    if (clazz == nullptr) {
