...
import com.blaze.pdfium.PdfTextPage
import com.blaze.pdfium.PdfiumCore
import com.blaze.pdfium.util.Size
import com.blaze.pdfium.util.SizeF
import com.blaze.pdfviewer.PDFView.Companion.FPDF_INCREMENTAL
import com.blaze.pdfviewer.PDFView.Companion.FPDF_NO_INCREMENTAL
import com.blaze.pdfviewer.PDFView.Companion.FPDF_REMOVE_SECURITY
import com.blaze.pdfviewer.exception.PageRenderingException
import com.blaze.pdfviewer.link.DefaultLinkHandler
import com.blaze.pdfviewer.link.LinkHandler
import com.blaze.pdfviewer.listener.Callbacks
import com.blaze.pdfviewer.listener.OnDrawListener
import com.blaze.pdfviewer.listener.OnErrorListener
import com.blaze.pdfviewer.listener.OnLoadCompleteListener
import com.blaze.pdfviewer.listener.OnLongPressListener
import com.blaze.pdfviewer.listener.OnPageChangeListener
import com.blaze.pdfviewer.listener.OnPageErrorListener
import com.blaze.pdfviewer.listener.OnPageScrollListener
import com.blaze.pdfviewer.listener.OnRenderListener
import com.blaze.pdfviewer.listener.OnTapListener
import com.blaze.pdfviewer.model.PagePart
import com.blaze.pdfviewer.scroll.ScrollHandle
import com.blaze.pdfviewer.source.AssetSource
import com.blaze.pdfviewer.source.ByteArraySource
import com.blaze.pdfviewer.source.DocumentSource
import com.blaze.pdfviewer.source.FileSource
import com.blaze.pdfviewer.source.InputStreamSource
import com.blaze.pdfviewer.source.UriSource
import com.blaze.pdfviewer.util.FitPolicy
import com.blaze.pdfviewer.util.PdfConstants
import com.blaze.pdfviewer.util.PdfUtils
import com.blaze.pdfviewer.util.ScrollDir
import com.blaze.pdfviewer.util.SnapEdge
import com.blaze.pdfviewer.util.State
import java.io.File
...
/**
 * PDF 查看器自定义视图组件
 * 提供功能丰富的 PDF 渲染和交互功能，包括缩放、滚动、页面切换等
 *
...
class PDFView(context: Context?, set: AttributeSet?) : RelativeLayout(context, set) {
    ...
    private var _searchHighlights: List<SearchHighlight> = emptyList()
    private var _activeSearchHighlightIndex: Int = -1
    private var _searchHighlightPaint: Paint? = null
    private var _activeSearchHighlightPaint: Paint? = null
    ...
    private fun drawSearchHighlights(canvas: Canvas) {
        ...
        _searchHighlights.forEachIndexed { index, highlight ->
            val pageIndex: Int = highlight.pageIndex
            if (pageIndex !in 0 until file.pagesCount) return@forEachIndexed

            val pageSize: SizeF = file.getScaledPageSize(pageIndex = pageIndex, zoom = _zoom)
            val pageWidth: Int = pageSize.width.toInt().coerceAtLeast(minimumValue = 1)
            val pageHeight: Int = pageSize.height.toInt().coerceAtLeast(minimumValue = 1)
            val (pageX, pageY) = if (_isSwipeVertical) {
                file.getSecondaryPageOffset(pageIndex = pageIndex, zoom = _zoom) to
                        file.getPageOffset(pageIndex = pageIndex, zoom = _zoom)
            } else {
                file.getPageOffset(pageIndex = pageIndex, zoom = _zoom) to
                        file.getSecondaryPageOffset(pageIndex = pageIndex, zoom = _zoom)
            }
            val paint: Paint? = if (index == _activeSearchHighlightIndex) {
                _activeSearchHighlightPaint
            } else {
                _searchHighlightPaint
            }

            highlight.rects.forEach { pageRect ->
                val mappedRect: RectF = file.mapRectToDevice(
                    pageIndex = pageIndex,
                    startX = pageX.toInt(),
                    startY = pageY.toInt(),
                    sizeX = pageWidth,
                ...
                if (mappedRect.width() > 0f && mappedRect.height() > 0f) {
                    paint?.let { canvas.drawRect(mappedRect, it) }
                }
            }
        }
    }
    ...
    fun setSearchHighlights(highlights: List<SearchHighlight>, activeIndex: Int = -1) {
        _searchHighlights = highlights
        _activeSearchHighlightIndex = activeIndex
        invalidate()
    }

    fun clearSearchHighlights() {
        _searchHighlights = emptyList()
        _activeSearchHighlightIndex = -1
        invalidate()
    }
    ...
    val pagesCount: Int get() = pdfFile?.pagesCount ?: 0
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        drawSearchHighlights(canvas = canvas)
    ...
    private fun initPDFView() {
        ...
        _searchHighlightPaint = Paint().apply {
            style = Paint.Style.FILL
            color = Color.argb(96, 255, 213, 79)
        }
        _activeSearchHighlightPaint = Paint().apply {
            style = Paint.Style.FILL
            color = Color.argb(168, 255, 160, 0)
        }
