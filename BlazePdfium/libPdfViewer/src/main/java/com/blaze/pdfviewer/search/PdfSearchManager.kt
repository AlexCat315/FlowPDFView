package com.blaze.pdfviewer.search
...
import com.blaze.pdfium.FindFlags
import com.blaze.pdfium.FindResult
import com.blaze.pdfium.PdfTextPage
import com.blaze.pdfviewer.PDFView
...
class PdfSearchManager(
    ...
    private fun syncHighlights() {
        if (searchMatches.isEmpty()) {
            pdfView.clearSearchHighlights()
            return
        }
        val highlights: List<PDFView.SearchHighlight> = searchMatches.map { result ->
            PDFView.SearchHighlight(
                pageIndex = result.pageIndex,
                rects = result.rect
            )
        }
        pdfView.setSearchHighlights(
            highlights = highlights,
            activeIndex = currentMatchPosition
        )
    }
